...
def split_log_content(log_content: str)-> str:
    ...
    logs_metadata={"log_size":len(log_content),"chunks":len(texts),"Approx_chunk_size":len(texts[0])}
    print(logs_metadata)
...
async def summarize_log(file: UploadFile = File(...)):
    ...
    for chunk in chunks[:10]:
        i+=1
        try:
            logging.info(f"Generating log summary for chunk{i}.")
            summary = summarize_chain.invoke({"log_content": chunk})
            print("chunk Summary len:",len(summary))
